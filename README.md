...
Для части изменений инструмент выполняет `update` без пересоздания ресурса. В текущей версии обновляются `name` и `labels` у базовых ресурсов, CIDR у `subnet`, правила у `security_group`, размер у `disk`, а также `name`, `labels`, `cores`, `memory_gb`, `preemptible` и `security_groups` у `instance`. При изменении `subnet.cidr`, `instance.cores` или `instance.memory_gb` затронутые VM временно останавливаются, обновление выполняется через SDK, а затем VM запускаются обратно, если до операции они были запущены. Изменения полей, которые безопаснее заменить целиком, например образ VM, пользователь SSH или boot disk, остаются `replace`.
